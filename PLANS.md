...
4. Deterministic checkout controls for origin, `businessId`, product/variant stock, Stripe Price IDs, delivery method, destination rate, goggle-with-helmet limit, and idempotency.
...
The initial release uses repository-managed inventory. Stripe Checkout is available only after server credentials and matching Price IDs are configured. There is no customer account, order database, or transactional inventory decrement in phase one.
