...
export default async function AdminLoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="page-shell admin-login"><form action={loginAction} className="admin-login__card"><p className="eyebrow eyebrow--accent">APEX MOTO OWNER</p><h1>Order desk.</h1><p>Private access to customer orders, fulfilment, stock, email delivery and refunds.</p><label htmlFor="admin-password">Owner password</label><input id="admin-password" name="password" type="password" autoComplete="current-password" required minLength={12} />{message ? <p className="form-message form-message--error" role="alert">{message}</p> : null}<button className="button button--primary button--wide" type="submit">Sign in</button></form></div>;
}
