    ...
    <div className="product-page">
      ...
      <section className="container product-information">
        ...
        <div className="product-information__panels">
          ...
          {product.specifications?.length ? <article><h3>Verified specifications</h3><dl>{product.specifications.map((specification) => <div key={specification.label}><dt>{specification.label}</dt><dd>{specification.value}</dd></div>)}</dl></article> : null}
          {product.inTheBox?.length ? <article><h3>In the box</h3><ul>{product.inTheBox.map((item) => <li key={item}>{item}</li>)}</ul></article> : null}
          {product.category !== "goggles" ? <article><h3>Helmet marking</h3>{product.certification?.verified ? <p><strong>{product.certification.standard}</strong>{product.certification.approvalNumber ? ` — ${product.certification.approvalNumber}` : ""}. The marking is visible in the supplied product image; it does not by itself confirm Australian road legality.</p> : <p>Check applicable helmet requirements for your intended use before riding.</p>}<Link href="/safety" className="text-link">Read more</Link></article> : null}
        </div>
      </section>
