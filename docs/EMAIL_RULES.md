...
All messages reply to `apexmotostore.au@gmail.com` and state the normal 12–48 hour support window. The live `ORDER_EMAIL_FROM` must be a Resend-verified sender. The exact private pickup address is used only when `PICKUP_ADDRESS_PRIVATE` is configured; otherwise the email truthfully says it will be supplied with the confirmed appointment.
