...
All messages reply to `max@apexmoto.com.au`, which is received through ImprovMX and forwarded to the store Gmail inbox, and state the normal 12–48 hour support window. The live `ORDER_EMAIL_FROM` uses the provider-verified `apexmoto.com.au` domain. Postmark is attempted first with open/link tracking disabled; the verified Resend account is used automatically if Postmark rejects or cannot accept the send. The exact private pickup address is used only when `PICKUP_ADDRESS_PRIVATE` is configured; otherwise the email truthfully says it will be supplied with the confirmed appointment.
